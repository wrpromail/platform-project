...
public class UserProjectSettingController {
    ...
    public Result search(
        ...
        // 根据 code 列表获取对应配置
        List<UserProjectSettingValueDTO> result =
                userProjectSettingService.getValuesByCodes(codes, projectId, userId);
    ...
    public Result update(@RequestHeader(GatewayHeader.PROJECT_ID) Integer projectId,
                         @RequestHeader(GatewayHeader.USER_ID) Integer userId,
                         @PathVariable("code") String code,
                         @RequestParam(value = "value") String value)
            throws CoreException {
        // 更新用户配置
        UserProjectSettingValueDTO userProjectSettingValueDTO =
                userProjectSettingService.updateUserProjectSettingValue(code, value, projectId, userId);
        return Result.success(userProjectSettingValueDTO);
    }
}
