...
public class InflectorUtil {
     ...
     *
     * @return Replaces a found pattern in a word and returns a transformed word. Null is pattern
     * does not match.
     */
    ...
    /**
     * Converts a camel case to underscore and then pluralizes. <p/> Example: "GrayDuck" is
     * converted to "gray_ducks".
     *
     ...
     *
     * @param underscore          underscore version of a word to converted to camel case.
     * @param capitalizeFirstChar set to true if first character needs to be capitalized, false if
     *                            not.
     * @return camel case version of underscore.
     */
    ...
    public static String getIdName(String tableName) {
        String idName = InflectorUtil.singularize(tableName) + "_id";
        return idName.toLowerCase();
    }

    /**
     * If a table name is made of two other table names (as is typical for many to many
     * relationships), this method retrieves a name of "another" table from a join table name. For
     * instance, if a source table is "payer" and the target is "player_game", then the returned
     * value will be "game".
     *
